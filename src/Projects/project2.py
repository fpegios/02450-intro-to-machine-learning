...
error_per_model = np.empty(K_out)  # error per model
...
for train_out_index, test_out_index in CV_out.split(X):
    ...
    y_est_test_out = dtc.predict(X_test_out)

    # we calculate the final error of each model
    error_per_model[k_out] = (sum(np.abs(y_est_test_out - Y_test_out)) / float(len(y_est_test_out))).mean();

    # Display results
    print('    =============================')
    print('    Best Depth: {0}'.format(best_depth[k_out]))
    print('    Error: {0}'.format(error_per_model[k_out]))
    print('\n')
    k_out += 1

error_dt = error_per_model.mean()
# knowing the error for the 5 folds, we find the mean of the errors and this is the final error of the method
print('=======================================')
print('Decision Tree Error: {0}'.format(error_dt))
print('=======================================')
print('\n')
...
error_mnr = error_per_model.mean()
# knowing the error for the 5 folds, we find the mean of the errors and this is the final error of the method
print('=======================================')
print('Decision Tree Error: {0}'.format(error_mnr))
print('=======================================')
print('\n')
